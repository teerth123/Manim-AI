...
exports.X = X;
exports.EmbeddingFun = EmbeddingFun;
const google_1 = require("@ai-sdk/google");
const ai_1 = require("ai");
const ai_2 = require("ai");
async function X({ prompt }) {
    const model = (0, google_1.google)('gemini-2.5-flash');
    const { text, reasoning } = await (0, ai_1.generateText)({
        model: model,
...
async function EmbeddingFun(prompt) {
    const model = google_1.google.textEmbedding(`text-embedding-004`);
    const { embedding } = await (0, ai_2.embed)({
        model,
        value: prompt,
    });
    return embedding;
